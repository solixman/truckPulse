...
describe("Tire Service", () => {
  ...
  test("creates a new tire", async () => {
    ...
    expect(Tire.create).toHaveBeenCalledWith({
      brand: "Michelin",
      size: "17",
      wearLevel: 10,
      status: "inStorage",
      lastReplacedAt: tireData.lastReplacedAt,
    });
  });
  ...
  Tire.find.mockReturnValue({
    ...
    skip: jest.fn().mockResolvedValue(tires),
  });
  ...
  test("updates a tire successfully", async () => {
    ...
    const updated = await tireService.update("id123", {
      brand: "Pirelli",
      wearLevel: 50,
    });

    expect(fakeTire.brand).toBe("Pirelli");
    expect(fakeTire.wearLevel).toBe(50);
  ...
  test("throws error if tire not found (update)", async () => {
    Tire.findById.mockResolvedValue(null);
    await expect(
      tireService.update("id123", { brand: "Pirelli" })
    ).rejects.toThrow("Tire not found");
  });
  ...
  test("throws error if deleting non-existent tire", async () => {
    ...
    await expect(tireService.deleteTire("id123")).rejects.toThrow(
      "Tire not found"
    );
  });
});
